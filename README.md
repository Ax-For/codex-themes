...
macOS 常驻控制器包含 Codex 更新后的自愈逻辑：启动阶段的瞬时失败会退避重试；原生启动的 Codex 会被重新接管为本机 CDP 模式；detached 重启失败不会永久耗尽重试资格。自愈不会自动下载新选择器或 CSS；Codex 更新 DOM 后仍需更新仓库并重新运行 macOS 安装器。
