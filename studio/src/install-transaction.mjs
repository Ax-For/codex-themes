...
const OWNED_TOP_LEVEL = Object.freeze([
  ...SOURCE_ENTRIES.map((entry) => entry.name),
  INSTALL_MARKER_NAME,
].sort());
const LEGACY_OWNED_TOP_LEVELS = Object.freeze([
  Object.freeze([...OWNED_TOP_LEVEL, "custom-pet"].sort()),
  Object.freeze([...OWNED_TOP_LEVEL, "custom-pet", "test"].sort()),
]);
...
export function isAcceptedOwnedTopLevel(topLevel, { allowLegacyLayout = false } = {}) {
  if (!Array.isArray(topLevel) || topLevel.some((name) => typeof name !== "string")) return false;
  const normalized = [...topLevel].sort();
  if (sameStringList(normalized, OWNED_TOP_LEVEL)) return true;
  return allowLegacyLayout
    && LEGACY_OWNED_TOP_LEVELS.some((layout) => sameStringList(normalized, layout));
}

async function validateOwnedTree(
  root,
  ...
  if (!isAcceptedOwnedTopLevel(topLevel, { allowLegacyLayout })) {
...
async function validateParticipantBeforeTree(participant, path, testMode) {
  ...
  if (participant.beforeKind === "owned") {
    return validateOwnedTree(path, participant.beforeManifestSha256, { allowLegacyLayout: true });
  }
...
export async function prepareInstallTree({
  ...
  if (existing !== null) {
    if (await pathInfo(join(targetRoot, INSTALL_MARKER_NAME))) {
      before = await validateOwnedTree(targetRoot, null, { allowLegacyLayout: true });
      beforeKind = "owned";
    } else {
