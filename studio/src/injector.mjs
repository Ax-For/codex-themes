...
export async function applySkin({
  ...
  const loadedThemeId = loadedTheme.manifest.id;
  const themeId = activeId === undefined ? loadedThemeId : activeId;
  if (
    themeId !== null &&
    !entries.some((entry) => entry.id === themeId)
  ) {
    throw new Error("activeId 必须是菜单中的主题或 null");
  }
